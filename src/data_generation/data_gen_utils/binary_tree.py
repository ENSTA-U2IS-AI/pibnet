# From https://github.com/merantix-momentum/multiscale-pde-operators
...
class TreeOpTransform:
    def __init__(self, k_hop_levels):
    ...
    def create_tree(self, points):
        ...
        while loop:
            for leaf in list(tree.traverse()):
                if leaf.point_indices.shape[0] < 2:
                    loop = False
                    break
    ...
    def __call__(self, pos):
        tree = self.create_tree(pos)
        edge_indices = self.create_edge_indices_simple(tree, self.k_hop_levels)

        # remove self loops
        for lvl in edge_indices:
            mask = edge_indices[lvl][0] != edge_indices[lvl][1]
            edge_indices[lvl] = edge_indices[lvl].T[mask].T

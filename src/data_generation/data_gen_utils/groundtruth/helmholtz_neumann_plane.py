...
def generate_groundtruth_helmholtz_neumann_plane(grid, index: int, sample: dict, cfg: Any, unit_sphere_points: np.ndarray = None):
    ...
    lhs = W = bempp_cl.api.operators.boundary.helmholtz.hypersingular(piecewise_const_space, piecewise_const_space, piecewise_const_space, k)
        
    @bempp_cl.api.complex_callable
    def dnu_u_inc(x, n, domain_index, result):
        phase = np.exp(1j * k * np.dot(inc_dir, x))
        result[0] = 1j * k * np.dot(inc_dir, n) * phase
        
    grid_fun = bempp_cl.api.GridFunction(piecewise_const_space, fun=dnu_u_inc)

    neumann_fun, info, iteration_count = bempp_cl.api.linalg.gmres(lhs, -grid_fun, tol=cfg.gmres_tolerance, return_iteration_count=True)
    coefs = neumann_fun.coefficients
    ...
    if cfg.generate_all_sol:
        ...
        # ff_sol = slp_ff.evaluate(neumann_fun)
        ...
        res = free_field + slp_pot.evaluate(neumann_fun)
...
def generate_groundtruth_helmholtz_neumann_plane_v2(grid, index: int, sample: dict, cfg: Any, unit_sphere_points: np.ndarray = None):
